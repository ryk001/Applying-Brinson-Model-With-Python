...
Original file is located at
    https://colab.research.google.com/drive/1-E7UtjC8w4Yf4s-EvDesNByvDtLRZnix
...
benchmark = pd.read_excel('/content/drive/MyDrive/EV+/程式 team/202301 Brinson Model/benchmark.xlsx')
portfolio_return = pd.read_excel('/content/drive/MyDrive/EV+/程式 team/202301 Brinson Model/portfolio_return.xlsx').fillna(0)
portfolio_weight = pd.read_excel('/content/drive/MyDrive/EV+/程式 team/202301 Brinson Model/portfolio_weight.xlsx').fillna(0)

"""## *Clean data*"""

# clean our portfolio data 
portfolio_r = pd.DataFrame()
portfolio_w = pd.DataFrame()

date_list = list(portfolio_return.date.astype(str).unique())
for i in date_list:
  df_r = portfolio_return[portfolio_return.date==i].drop(['date'], axis=1).T.reset_index().set_axis(['sector', 'return'], axis=1)
  df_r['date'] = i
  portfolio_r = portfolio_r.append(df_r[['date', 'sector', 'return']], ignore_index=True)

  df_w = portfolio_weight[portfolio_weight.date==i].drop(['date', 'Total_net_long'], axis=1).T.reset_index().set_axis(['sector', 'weight'], axis=1)
  df_w['date'] = i
  portfolio_w = portfolio_w.append(df_w[['date', 'sector', 'weight']], ignore_index=True)

portfolio = pd.merge(portfolio_r,portfolio_w, on=['date', 'sector']).rename(columns={'return': 'r_portfolio', 'weight': 'w_portfolio'}).round({'r_portfolio': 5, 'w_portfolio': 5})
portfolio['date'] = portfolio.date.astype(str)

# recalculate return
condition = (portfolio.w_portfolio != 0) & (portfolio.sector != 'Total')
portfolio.loc[condition, ['r_portfolio']] = (portfolio[condition].r_portfolio/portfolio[condition].w_portfolio).fillna(0).round(5)

# clean benchmark data
try:
  benchmark['date'] = benchmark.date.astype(str)
  benchmark.date = benchmark.date.apply(lambda x : None if x=="NaT" else x)
  benchmark = benchmark.fillna(method="ffill").drop(['week'], axis=1)[['date', 'sector', 'r_benchmark', 'w_benchmark']].round({'r_benchmark': 5, 'w_benchmark': 5})
except:
  pass

# recalculate return
# benchmark.loc[benchmark['sector']=='Total', ['r_benchmark']] = benchmark[benchmark['sector']!='Total'].assign(col=benchmark.r_benchmark*benchmark.w_benchmark).groupby('date',as_index=False).col.sum().round(5).col.to_list()

benchmark_and_portfolio = pd.merge(benchmark,portfolio, on=['date', 'sector'])
...
# # 匯出到google sheet
# def store_to_drive():
#   title = 'benchmark_and_portfolio'
#   gc.create(title)
#   sheet = gc.open(title).sheet1
#   set_with_dataframe(sheet, benchmark_and_portfolio)
...
brinson_sector_analysis

"""## *download*"""

def download_as_excel(file, name):
  file.to_excel(name+'.xlsx', encoding = 'utf-8-sig') 
  files.download(name+'.xlsx')


download_as_excel(brinson_model, 'brinson_model')
download_as_excel(cumulative_return, 'cumulative_return')
download_as_excel(brinson_sector_analysis, 'brinson_sector_analysis')
